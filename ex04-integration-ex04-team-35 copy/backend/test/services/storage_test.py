...
def test_delete_user_unknown_pid(storage_service: StorageService):
    """Test deleting a user that does not exist."""
    with pytest.raises(Exception):
        storage_service.delete_user(710453085)


def test_delete_user_known_pid(storage_service: StorageService):
    """Test deleting a known user."""
    pid = 710453084
    user = User(pid=pid, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    
    assert storage_service.get_user_by_pid(pid) is user
    
    storage_service.delete_user(pid)
    
    assert storage_service.get_user_by_pid(pid) is None


def test_delete_user_and_associated_checkins(storage_service: StorageService):
    """Test deleting a user and their associated check-ins."""
    pid = 710453084
    user = User(pid=pid, first_name="Kris", last_name="Jordan")
    storage_service.create_registration(user)
    storage_service.create_checkin(pid)
    
    assert len(storage_service.get_checkins()) == 1
    
    storage_service.delete_user(pid)
    
    assert storage_service.get_user_by_pid(pid) is None
    assert len(storage_service.get_checkins()) == 0
